...
const props = withDefaults(defineProps(), {
  ...
  drag: { // 是否支持拖拽上传 ==> 非必传（默认为 true）
    type: Boolean,
    default: true
  },
  disabled: { // 是否禁用上传组件 ==> 非必传（默认为 false）
    type: Boolean,
    default: false
  },
  fileSize: { // 图片大小限制 ==> 非必传（默认为 5M）
    type: Number,
    default: 5
  },
  fileType: { // 图片类型限制==> 非必传
  ...
  width: { // 组件宽度 ==> 非必传（默认为 150px）
    type: String,
    default: '150px'
  },
  height: { // 组件高度 ==> 非必传（默认为 150px）
    type: String,
    default: '150px'
  },
  borderRadius: { // 组件边框圆角 ==> 非必传（默认为 8px）
    type: String,
    default: '8px'
  },
  api: {
